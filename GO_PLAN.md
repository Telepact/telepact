...
`poetry run python -m pytest -k 'test_binary_case[py'`.
...
`poetry run python -m pytest -k 'test_binary_case[py-0]' -s -vv`.


# Goal

Your goal is to, from the `test/runner` directory, get `poetry run python -m pytest -k 'test_binary_case[go'` to pass.
...
`poetry run python -m pytest -k 'test_binary_case[go-0]' -s -vv`.

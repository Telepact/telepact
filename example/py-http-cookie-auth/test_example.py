...
def test_cookie_auth_example_runs_end_to_end() -> None:
    ...
    try:
        ...
        unauthenticated_body = unauthenticated[INDEX_MESSAGE_BODY]
        assert unauthenticated_body.get('Ok_') is None
        assert any(key.lower().startswith('error') for key in unauthenticated_body)
        ...
        assert authenticated[INDEX_MESSAGE_BODY]['Ok_']['userId'] == 'user-123'
    finally:

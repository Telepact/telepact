    ...
    for line in source.read_text(encoding="utf-8").splitlines():
        ...
        if heading:
            ...
            if len(heading.group(1)) == 2:
                current_group = NavGroup(heading=title)
                groups.append(current_group)
                current_subgroup = None
            elif current_group is not None:
                current_subgroup = NavSubgroup(heading=title)
        ...
        if current_subgroup is not None:
            current_subgroup.items.append(nav_link)
        else:
            current_group.items.append(nav_link)
...
def nav_groups() -> list[NavGroup]:
    groups = parse_nav_groups(REPO_ROOT / "doc" / "index.md")
    for group in groups:
        if len(group.items) != 1 or group.subgroups:
            continue
        item = group.items[0]
...
def render_nav_link(current: Page, pages: dict[Path, Page], resources: set[Path], item: NavLink) -> str:
    target_path, fragment = resolve_local_target(item.source, item.target)
    active = ""
    href = item.target
    if target_path is not None and target_path.suffix == ".md":
        ...
        if page is not None:
            ...
            href = f"{REPO_URL}/blob/main/{repo_rel(target_path)}"
    else:

...
DOCS_CONCEPTS = DIST_DIR / "docs" / "concepts" / "index.html"
...
class DocsSearchTest(unittest.TestCase):
    ...
    def test_docs_shell_includes_search_ui_assets(self) -> None:
        ...
        self.assertIn('href="assets/docs-search.css"', html)
        self.assertIn('src="assets/docs-search.js"', html)
        self.assertIn('data-search-index="assets/search-index.json"', html)
        self.assertIn('data-docs-search-open', html)
        self.assertIn('id="docs-search-modal"', html)

        nested_html = DOCS_CONCEPTS.read_text(encoding="utf-8")
        self.assertIn('href="../assets/docs-search.css"', nested_html)
        self.assertIn('src="../assets/docs-search.js"', nested_html)
        self.assertIn('data-search-index="../assets/search-index.json"', nested_html)

    def test_search_index_covers_representative_doc_locations(self) -> None:
        indexed_urls = {entry["url"] for entry in self.index}
        self.assertIn("./", indexed_urls)
        self.assertIn("concepts/#schema-writing-guide", indexed_urls)
        self.assertIn("learn-by-example/#00-installation", indexed_urls)
        self.assertIn("lib-and-sdk-survey/#cli", indexed_urls)
        self.assertIn("examples/full-stack/#layout", indexed_urls)

    def test_search_index_entries_include_prose_snippets(self) -> None:
        entries_by_url = {entry["url"]: entry for entry in self.index}
        installation = entries_by_url["learn-by-example/#00-installation"]
        cli = entries_by_url["lib-and-sdk-survey/#cli"]

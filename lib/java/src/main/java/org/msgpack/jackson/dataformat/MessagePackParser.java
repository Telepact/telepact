...
public class MessagePackParser
    ...
    private static final ThreadLocal<Tuple<Object, MessageUnpacker>> messageUnpackerHolder = new ThreadLocal<Tuple<Object, MessageUnpacker>>();
    ...
    private MessagePackParser(IOContext ctxt,
        ...
        if (messageUnpackerTuple == null) {
            ...
            // MessagePackParser needs to use the MessageUnpacker that has the same
            // InputStream
            // since it has buffer which has loaded the InputStream data ahead.
            // However, it needs to call MessageUnpacker#reset when the source is different
    ...
    public JsonToken nextToken()
        ...
        switch (valueType) {
            ...
            case EXTENSION:
                ...
                extensionTypeValue = new MessagePackExtensionType(header.getType(),
                        messageUnpacker.readPayload(header.getLength()));
                if (parsingContext.inObject() && _currToken != JsonToken.FIELD_NAME) {

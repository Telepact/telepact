...
public class CatchErrorCollisions {
    public static void catchErrorCollisions(Map<String, List<Object>> msgPactSchemaNameToPseudoJson, Set<String> errorKeys,
        ...
        for (var i = 0; i < errorKeysList.size(); i++) {
            for (var j = i + 1; j < errorKeysList.size(); j++) {
                ...
                var otherMsgPactSchemaPseudoJson = msgPactSchemaNameToPseudoJson.get(otherDocumentName);

                var def = (Map<String, Object>) msgPactSchemaPseudoJson.get(index);
                var otherDef = (Map<String, Object>) otherMsgPactSchemaPseudoJson.get(otherIndex);

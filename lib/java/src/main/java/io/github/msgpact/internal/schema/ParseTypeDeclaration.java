    ...
    static VTypeDeclaration parseTypeDeclaration(
        ...
        if (typeDeclarationArray.isEmpty()) {
            throw new MsgPactSchemaParseError(List.of(new SchemaParseFailure(ctx.documentName, path,
                    "EmptyArrayDisallowed", Map.of())), ctx.msgPactSchemaDocumentNamesToJson);
        }
        ...
        if (!matcher.find()) {
            throw new MsgPactSchemaParseError(List.of(new SchemaParseFailure(ctx.documentName, basePath,
        ...
        for (final var e : givenTypeParameters) {
            ...
            } catch (MsgPactSchemaParseError e2) {

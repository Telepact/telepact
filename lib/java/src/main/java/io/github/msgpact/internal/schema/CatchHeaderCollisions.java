...
import static io.github.msgpact.internal.schema.GetPathDocumentCoordinatesPseudoJson.getPathDocumentCoordinatesPseudoJson;
...
public class CatchHeaderCollisions {
    ...
    public static void catchHeaderCollisions(
            Map<String, List<Object>> msgPactSchemaNameToPseudoJson,
        ...
        for (int i = 0; i < headerKeysList.size(); i++) {
            for (int j = i + 1; j < headerKeysList.size(); j++) {
                ...
                List<Object> msgPactSchemaPseudoJson = msgPactSchemaNameToPseudoJson.get(documentName);
                List<Object> otherMsgPactSchemaPseudoJson = msgPactSchemaNameToPseudoJson.get(otherDocumentName);

                Map<String, Object> def = (Map<String, Object>) msgPactSchemaPseudoJson.get(index);
                Map<String, Object> otherDef = (Map<String, Object>) otherMsgPactSchemaPseudoJson.get(otherIndex);

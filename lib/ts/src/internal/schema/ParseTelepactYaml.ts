...
function parseYamlText(text: string): { value: any; locations: Record<string, Coordinates> } {
    ...
    return { value: parsed.value, locations };
}

export function parseTelepactYaml(text: string): { canonicalJsonText: string; locator: DocumentLocator } {
    const parsed = parseYamlText(text);
    return {
        canonicalJsonText: JSON.stringify(parsed.value),
        locator: (path: Path) => parsed.locations[serializePath(path)] ?? { row: 1, col: 1 },
    };
}

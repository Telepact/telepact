...
func (c *DefaultBinaryEncodingCache) Add(checksum int, binaryEncodingDescriptor map[string]any) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	encoding, err := BinaryEncodingFromNegotiationDescriptor(checksum, binaryEncodingDescriptor)
	if err != nil {
		return
	}

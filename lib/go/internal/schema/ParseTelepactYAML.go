...
func parseYAMLSequence(lines []string, startIndex int, indent int, path []any, locations map[string]map[string]any) (*yamlNode, error) {
	...
	for {
		...
		if findYAMLMappingColon(valueText) >= 0 {
		...
		parsedValue, err := parseYAMLValueText(lines, info, indent, itemPath, valueText, locations)
		if err != nil {
			return nil, err
		}
...
func parseYAMLMapEntries(lines []string, startIndex int, indent int, path []any, locations map[string]map[string]any, initialEntry map[string]any) (*yamlNode, error) {
	...
	for {
		...
		parsedValue, err := parseYAMLValueText(lines, info, indent, keyPath, entry["value_text"].(string), locations)
		if err != nil {
			return nil, err
		}
...
func parseYAMLValueText(lines []string, info *yamlLineInfo, currentIndent int, path []any, valueText string, locations map[string]map[string]any) (*yamlNode, error) {
	switch valueText {
	case "{}":
...
func rejectUnsupportedYAML(line string) error {
	...
	if hasUnsupportedYAMLFlowValue(trimmed) {
		return fmt.Errorf("Non-empty flow collections are not supported")
	}
	return nil
}

func hasUnsupportedYAMLFlowValue(text string) bool {
	if (strings.HasPrefix(text, "[") && text != "[]") || (strings.HasPrefix(text, "{") && text != "{}") {
		return true
	}

	if strings.HasPrefix(text, "- ") {
		return hasUnsupportedYAMLFlowValue(strings.TrimLeft(text[2:], " "))
	}

	if colonIndex := findYAMLMappingColon(text); colonIndex >= 0 {
		return hasUnsupportedYAMLFlowValue(strings.TrimLeft(text[colonIndex+1:], " "))
	}
...
func parseYAMLInlineEntry(text string, baseColumn int) (map[string]any, error) {
	...
	valueText := strings.TrimLeft(trimmed[colonIndex+1:], " ")

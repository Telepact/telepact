...
from ...MsgPactError import MsgPactError
...
def validate_result(result_union_type: 'VUnion', error_result: object) -> None:
    ...
    if new_error_result_validation_failures:
        raise MsgPactError(
            f"Failed internal msgPact validation: {map_validation_failures_to_invalid_field_cases(new_error_result_validation_failures)}"
        )

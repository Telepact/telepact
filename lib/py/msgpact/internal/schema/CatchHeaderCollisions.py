...
def catch_header_collisions(
    msgpact_schema_name_to_pseudo_json: Dict[str, List[object]],
    header_keys: Set[str],
    ...
    for i in range(len(header_keys_list)):
        for j in range(i + 1, len(header_keys_list)):
            ...
            msgpact_schema_pseudo_json = msgpact_schema_name_to_pseudo_json[document_name]
            other_msgpact_schema_pseudo_json = msgpact_schema_name_to_pseudo_json[
                other_document_name]

            def_ = cast(dict[str, object], msgpact_schema_pseudo_json[index])
            other_def = cast(dict[str, object],
                             other_msgpact_schema_pseudo_json[other_index])
    ...
    if parse_failures:
        raise MsgPactSchemaParseError(parse_failures, document_names_to_json)

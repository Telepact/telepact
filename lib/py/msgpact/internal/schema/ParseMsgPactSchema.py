...
def parse_vers_api_schema(
    msgpact_schema_document_names_to_json: dict[str, str],
) -> 'MsgPactSchema':
    ...
    ordered_document_names = sorted(
        list(msgpact_schema_document_names_to_json.keys()))

    u_api_schema_document_name_to_pseudo_json: dict[str, list[object]] = {}

    for document_name, msgpact_schema_json in msgpact_schema_document_names_to_json.items():
        try:
            msgpact_schema_pseudo_json_init = json.loads(msgpact_schema_json)
        except json.JSONDecodeError as e:
            raise MsgPactSchemaParseError(
                [SchemaParseFailure(document_name, [], "JsonInvalid", {})], msgpact_schema_document_names_to_json) from e

        if not isinstance(msgpact_schema_pseudo_json_init, list):
            this_parse_failure = get_type_unexpected_parse_failure(
                document_name, [], msgpact_schema_pseudo_json_init, "Array"
            )
            raise MsgPactSchemaParseError(
                this_parse_failure, msgpact_schema_document_names_to_json)

        u_api_schema_document_name_to_pseudo_json[document_name] = msgpact_schema_pseudo_json_init
        ...
        for definition in u_api_schema_pseudo_json:
            ...
            try:
                schema_key = find_schema_key(
                    document_name, def_, index, msgpact_schema_document_names_to_json)
                ...
                if matching_schema_key is not None:
                    ...
                    document_json = msgpact_schema_document_names_to_json[other_document_name]
    ...
    if parse_failures:
        raise MsgPactSchemaParseError(
            parse_failures, msgpact_schema_document_names_to_json)
        ...
        try:
            get_or_parse_type(
                ...
                ParseContext(
                    ...
                    msgpact_schema_document_names_to_json,
    ...
    try:
        catch_error_collisions(u_api_schema_document_name_to_pseudo_json,
                               error_keys, schema_keys_to_index, schema_keys_to_document_names, msgpact_schema_document_names_to_json)
    except MsgPactSchemaParseError as e:
        parse_failures.extend(e.schema_parse_failures)
    ...
    for error in errors:
        try:
            apply_error_to_parsed_types(
                error, parsed_types, schema_keys_to_document_names, schema_keys_to_index, msgpact_schema_document_names_to_json)
        except MsgPactSchemaParseError as e:
            parse_failures.extend(e.schema_parse_failures)
    ...
    try:
        catch_header_collisions(u_api_schema_document_name_to_pseudo_json, header_keys,
                                schema_keys_to_index, schema_keys_to_document_names, msgpact_schema_document_names_to_json)
    except MsgPactSchemaParseError as e:
        parse_failures.extend(e.schema_parse_failures)

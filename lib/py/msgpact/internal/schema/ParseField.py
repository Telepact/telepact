...
def parse_field(path: list[object], field_declaration: str, type_declaration_value: object,
    ...
    if not matcher:
        ...
        raise MsgPactSchemaParseError([SchemaParseFailure(ctx.document_name, final_path,
                                                          "KeyRegexMatchFailed",
                                                          {"regex": regex_string})], ctx.msgpact_schema_document_names_to_json)
    ...
    if not isinstance(type_declaration_value, list):
        raise MsgPactSchemaParseError(get_type_unexpected_parse_failure(
            ctx.document_name,
            this_path,
            type_declaration_value,
            "Array"), ctx.msgpact_schema_document_names_to_json)

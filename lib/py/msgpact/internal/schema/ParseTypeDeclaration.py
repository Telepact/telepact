...
def parse_type_declaration(path: list[object], type_declaration_array: list[object],
    ...
    if not type_declaration_array:
        raise MsgPactSchemaParseError(
            [SchemaParseFailure(ctx.document_name, path, "EmptyArrayDisallowed", {})], ctx.msgpact_schema_document_names_to_json)
    ...
    if not isinstance(base_type, str):
        ...
        raise MsgPactSchemaParseError(
            this_parse_failures, ctx.msgpact_schema_document_names_to_json)
    ...
    if not matcher:
        raise MsgPactSchemaParseError([SchemaParseFailure(
            ctx.document_name, base_path, "StringRegexMatchFailed", {"regex": regex_string})], ctx.msgpact_schema_document_names_to_json)
    ...
    if type_.get_type_parameter_count() != given_type_parameter_count:
        raise MsgPactSchemaParseError([SchemaParseFailure(ctx.document_name, path, "ArrayLengthUnexpected",
                                                          {"actual": len(type_declaration_array),
                                                           "expected": type_.get_type_parameter_count() + 1})], ctx.msgpact_schema_document_names_to_json)
    ...
    for index, e in enumerate(given_type_parameters, start=1):
        ...
        except MsgPactSchemaParseError as e2:
            parse_failures.extend(e2.schema_parse_failures)

    if parse_failures:
        raise MsgPactSchemaParseError(
            parse_failures, ctx.msgpact_schema_document_names_to_json)

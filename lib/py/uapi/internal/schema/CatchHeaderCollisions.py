...
def catch_header_collisions(
    ...
    header_keys_list.sort(key=lambda k: (schema_keys_to_document_names[k], keys_to_index[k]))
    ...
    for i in range(len(header_keys_list)):
        for j in range(i + 1, len(header_keys_list)):
            ...
            other_def = cast(dict[str, object], other_uapi_schema_pseudo_json[other_index])

            header_def = cast(dict[str, object], def_[def_key])
            other_header_def = cast(dict[str, object], other_def[other_def_key])

            header_collisions = [k for k in header_def if k in other_header_def]
            for header_collision in header_collisions:
                ...
                this_location = get_path_document_coordinates_pseudo_json(this_path, this_document_json)
                parse_failures.append(
                    SchemaParseFailure(
                        other_document_name,
                        [other_index, other_def_key, header_collision],
                        'PathCollision',
                        {'document': document_name, 'path': this_path, 'location': this_location},
                    )
                )
            ...
            res_header_collisions = [k for k in res_header_def if k in other_res_header_def]

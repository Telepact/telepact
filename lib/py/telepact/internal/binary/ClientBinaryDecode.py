...
def client_binary_decode(message: list[object], binary_encoding_cache: 'BinaryEncodingCache',
    ...
    if "@enc_" in headers:
        ...
        pack_sites_header = cast(list[list[object]], headers.get("@encp_", []))
        binary_encoding_cache.add(binary_checksum, binary_encoding, pack_sites_header)
    ...
    message_body = decode_body(encoded_message_body, binary_encoder, headers.get("@pac_") is True)

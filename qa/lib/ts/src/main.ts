...
function startSchemaTestServer(connection: NatsConnection, registry: Registry, apiSchemaPath: string, frontdoorTopic: string, config?: Record<string, any>): Subscription {
    const json = fs.readFileSync(apiSchemaPath, 'utf-8');
    const uApi: UApiSchema = UApiSchema.fromJson(json);
    ...
    const handler = async (requestMessage: Message): Promise<Message> => {
        ...
        try {
            ...
            console.error(e);
            return new Message({}, { 'ErrorValidationFailure': { 'cases': (e as UApiSchemaParseError).schemaParseFailuresPseudoJson } });
        }
    };
...
function startTestServer(connection: NatsConnection, registry: Registry, apiSchemaPath: string, frontdoorTopic: string, backdoorTopic: string): Subscription {
    const json = fs.readFileSync(apiSchemaPath, 'utf-8');
    let uApi: UApiSchema = UApiSchema.fromJson(json);
    const alternateUApi: UApiSchema = UApiSchema.extend(uApi, `
        [
            {

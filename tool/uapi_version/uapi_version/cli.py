...
def depset(version: str) -> None:
    if os.path.exists("pom.xml"):
        ...
        tree.write("pom.xml", xml_declaration=True,
                   encoding='utf-8', pretty_print=True)
        click.echo(f"Set uapi dependency to version {version} in pom.xml")
    elif os.path.exists("package.json"):
        with open("package.json", "r") as f:
            data = json.load(f)
        if "dependencies" in data and "uapi" in data["dependencies"]:
            ...
            with open("package.json", "w") as f:
                json.dump(data, f, indent=2)
            click.echo(
                f"Set uapi dependency to version {version} in package.json")
        else:
            click.echo("uapi dependency not found in package.json")
    elif os.path.exists("pyproject.toml"):
        with open("pyproject.toml", "r") as f:
            data = toml.load(f)
        if "dependencies" in data["project"] and "uapi" in data["project"]["dependencies"]:
            data["project"]["dependencies"]["uapi"] = version
            with open("pyproject.toml", "w") as f:
                toml.dump(data, f)
            click.echo(

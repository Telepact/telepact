...
def binary_client_server_proc(loop, transport_client, dispatcher_server):
    ...
    try:
        async def warmup():
            request = [{}, {'fn.ping_': {}}]
            await verify_client_case(transport_client, request, None, *topics)
...
def test_binary_client_server_case(loop, binary_client_server_proc, transport_client, name, req, res):
    topics = binary_client_server_proc

    async def t():
        await verify_client_case(transport_client, dc(req), dc(res), *topics, assert_binary=True)

    loop.run_until_complete(t())


def test_pbinary_client_server_case(loop, binary_client_server_proc, transport_client, name, req, res):
    topics = binary_client_server_proc

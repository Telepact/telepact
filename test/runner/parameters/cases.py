...
def _index_entries(definitions: list[dict[str, object]], include_internal: bool) -> list[dict[str, object]]:
    entries: list[dict[str, object]] = []
    for definition in definitions:
        schema_key = _schema_key(definition)
...
def _definition_closure(definitions: list[dict[str, object]], names: set[str]) -> list[dict[str, object]]:
    return [
        definition
        for definition in definitions
        if _schema_key(definition) in names
    ]
...
cases = {
    ...
    'api': [
        ...
        [[{}, {'fn.index_': {}}], [{}, {'Ok_': {'functions': _index_entries(_EXAMPLE_PUBLIC_SCHEMA, False)}}]],
        [[{}, {'fn.index_': {'includeInternal!': True}}], [{}, {'Ok_': {'functions': _index_entries(_EXAMPLE_FULL_SCHEMA, True)}}]],
        [[{}, {'fn.def_': {'name': 'fn.test'}}], [{}, {'Ok_': {'definitions': _definition_closure(
            _EXAMPLE_FULL_SCHEMA,
            {'fn.test', 'struct.Value', 'struct.ExStruct', 'union.ExUnion', 'fn.example', '_ext.Select_'},
        )}}]],

...
def client_server_proc(loop, transport_client, dispatcher_server):
...
def test_client_server_case(loop, client_server_proc, transport_client, name, req, res):
    topics = client_server_proc

    async def t():
        await verify_client_case(transport_client, dc(req), dc(res), *topics)

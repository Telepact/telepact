...
class Scenario:
    ...
    @property
    def key(self) -> str:
        return f"{self.data_shape}.{self.collection_shape}.{self.method}"
...
def iter_scenarios(
    data_shapes: Iterable[str] = DATA_SHAPES,
    collection_shapes: Iterable[str] = COLLECTION_SHAPES,
    methods: Iterable[str] = METHODS,
) -> Iterable[Scenario]:
    for data_shape, collection_shape, method in product(
        data_shapes,
        collection_shapes,
        methods,
    ):
        yield Scenario(data_shape, collection_shape, method)
...
def write_summary_markdown(path: Path, summarized: list[dict[str, Any]]) -> None:
    ...
    for row in summarized:
        metrics = row["metrics"]
        total_codec_time_ms = (
            metrics["requestSerializationTimeNs"]["median"]
            + metrics["requestDeserializationTimeNs"]["median"]
            + metrics["responseSerializationTimeNs"]["median"]
            + metrics["responseDeserializationTimeNs"]["median"]
        ) / 1_000_000.0
        lines.append(
            f"| {row['language']} | {row['dataShape']} | {row['collectionShape']} | {row['method']} | "
            f"{metrics['serializedRequestSizeBytes']['median']:.2f} | {metrics['serializedResponseSizeBytes']['median']:.2f} | {total_codec_time_ms:.4f} |"
        )
    path.write_text("\n".join(lines) + "\n")
